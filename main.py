...
class Entries:
    ...
    # adding entries from a json file (добавление текстовых полей из json-файла)
    ...
    # calling a modal window before deleting a entry (вызов модального окна перед удалением текстового поля)
    def ask_delete_entry(self):
        ...
        if active_entry in self.entries_list:
            if not name_of_function or name_of_function.isspace():
                ...
                return
            mw1 = ModalWindow(self.parent_window, title='ПРЕДУПРЕЖДЕНИЕ', labeltext='Вы точно хотите удалить функцию?')
            ok_button = Button(master=mw1.top, text='OK', command=mw1.cancel)
            mw1.add_button(ok_button)
            ok_button.bind('<Button-1>', lambda event: self.delete_entry(active_entry))
            self.parent_window.wait_window(mw1.top)
...
class App(Tk):
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Load json", command=self.commands.get_command_by_name('load_json_file'))
...
if __name__ == "__main__":
    ...
    commands_main.add_command('load_json_file', commands_main.load_json_file)
