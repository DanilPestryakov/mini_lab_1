...
class Entries:
    ...
    def __clean_focused_entry(self):
        self.focused_entry.delete(0, END)
        # redrawing plot button
        plot_button = self.parent_window.get_button_by_name('plot')
        if plot_button:
            plot_button.pack_forget()
    ...
    def clean_focused_entry(self):
        if self.focused_entry is None:
            return
        if len(self.focused_entry.get()) > 0:
            self.__modal_on_cleaning_nonempty_entry()

    def __modal_on_cleaning_nonempty_entry(self):
        mw = ModalWindow(self.parent_window, title='Очистка непустой строки', labeltext='Строка непуста, '
                                                                              'Вы действительно хотите очистить ее?')

        def on_click(is_yes):
            def func(*args, **kwargs):
                mw.cancel()
                if is_yes:
                    self.__clean_focused_entry()
            return func
...
class Commands:
    ...
    def delete_focused_entry(self, *args, **kwargs):
        self.__forget_canvas()
        self.__forget_navigation()
        self.parent_window.entries.__modal_on_deleting_nonempty_entry()

    def clean_focused_entry(self, *args, **kwargs):
        self.__forget_canvas()
        self.__forget_navigation()
        self.parent_window.entries.clean_focused_entry()
...
if __name__ == "__main__":
    ...
    commands_main.add_command('clean_focused_entry', commands_main.clean_focused_entry)
    ...
    app.add_button('clean_focused_entry', 'Очистить выбранное поле', 'clean_focused_entry', hot_key='<Control-l>')
