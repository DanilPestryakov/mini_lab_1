...
class Entries:
    ...
    def ask_delete_entry(self):
        active_entry = self.parent_window.focus_get()
        try:
            name_of_function = active_entry.get()
        #если не выбрана функция для удаления
        except AttributeError:
            error_mw = ModalWindow(self.parent_window, title='Выберите функцию', labeltext='Пожалуйста, щелкните ЛКМ по функции, которую Вы бы хотели удалить с графика')
            ok_button = Button(master=error_mw.top, text='OK', command = error_mw.cancel)
            error_mw.add_button(ok_button)
        if active_entry in self.entries_list:
            if not name_of_function or name_of_function.isspace():
                self.delete_entry(active_entry)
                return
            trigger_mw = ModalWindow(self.parent_window, title='Подтвердите выбор', labeltext='Удалить функцию с графика?')
            ok_button = Button(master=trigger_mw.top, text='OK', command=trigger_mw.cancel)
            trigger_mw.add_button(ok_button)
            ok_button.bind('<Button-1>', lambda event: self.delete_entry(active_entry))
            self.parent_window.wait_window(trigger_mw.top)
    ...
    def add_text(self, list_of_func):
    ...
    # deleting an entry (удаление текстового поля)
    def delete_entry(self, active_entry):
...
class Commands:
    ...
    def load(self):
        ...
        self.parent_window.entries.add_text(self._state.list_of_function)
        self.plot()
    ...
    def delete_function(self, *args, **kwargs):
        self.parent_window.entries.ask_delete_entry()
        self.__forget_canvas()
        self.__forget_navigation()
        self.plot()
...
if __name__ == "__main__":
    ...
    commands_main.add_command('delete_function', commands_main.delete_function)
    ...
    app.add_button('delete_function', 'Удалить функцию', 'delete_function', hot_key='<Control-q>')
