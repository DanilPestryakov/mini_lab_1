#!C:\Users\Tioma\Desktop\minilab\venv\Scripts\python.exe
...
class Entries:
	...
	def load_entries(self, list_of_function):
		...
		for func in list_of_function:
			...
			new_entry = Entry(self.parent_window, textvariable=entry_text)
			new_entry.icursor(0)
			new_entry.focus()
			entry_text.set(func)
			new_entry.pack()
	...
	def delete_entry(self):
		if self.__last_focused_entry__ in self.entries_list:
			self.__last_focused_entry__.pack_forget()
			self.entries_list.remove(self.__last_focused_entry__)
			
		for entry in self.entries_list:
			if entry.get() and self.parent_window.commands.is_canvas_plotted():
				self.parent_window.commands.plot()
				break
		else:
			self.parent_window.commands.hide_plot()
	...
	def delete_entry_click(self):
		for entry in self.entries_list:
			if entry == entry.focus_get():
				break
		if entry.get():
			#__last_focused_entry__ нужно, потому что при нажатии "отмена" фокус как-то сбивается на последний элемент
			self.__last_focused_entry__ = entry
			mw = ModalWindow(self.parent_window, title='Удаление непустой строки', labeltext='Вы пытаетесь удалить непустую строку')
			cancel_button = Button(master=mw.top, text='Отмена', command=mw.cancel)
			ok_button = Button(master=mw.top, text='Да, удалить', command=mw.delete_entry)
...
class Commands:
	class State:
		...
		def load_state(self):
			file_input = askopenfile(defaultextension=".json")
			if file_input is not None:
				self.list_of_function = json.loads(file_input.read())["list_of_function"]
			return self
	...
	def is_canvas_plotted(self):
		return self.__figure_canvas is not None
	...
	def plot(self, *args, **kwargs):
		...
		for entry in self.parent_window.entries.entries_list:
			...
			if is_not_blank(get_func_str):
				...
				# при добавлении поле по умолчанию пустое, чтобы лишний раз не мелькало всплывающее окно, я добавил условие проверки аргументов на наличие replotted_by_adding_entry
				if self.__empty_entry_counter == 0 and not "replotted_by_adding_entry" in kwargs.keys():
		...
		#мне показалось нелогичным что нельзая плотить без добавления или удаления
		#поэтому закомментировал
		#так можно просто редактировать уже существующие формулы
		#if plot_button:
		#	plot_button.pack_forget()

	def add_func(self, *args, **kwargs):
		#мне кажется не логичным удалять график при добавлении поля
		self.__forget_canvas()
		self.__forget_navigation()
		self.parent_window.entries.add_entry()
		# чтобы не плотил если мы нажимаем кнопку добавить из состояния когда графика нет
		if self.is_canvas_plotted():
			self.plot(replotted_by_adding_entry = True)

	def delete_func(self, *args, **kwargs):
		self.parent_window.entries.delete_entry_click()
	...
	def load_from_file(self):
		...
		self.parent_window.entries.load_entries(self._state.list_of_function)
		self.plot()
		return self
...
class ModalWindow:
	...
	#я не понял как по другому вернуть ответ прямо в функцию delete_entry_click в зависимости от того, решил ли юзер отменить или действительно удалить 
	def delete_entry(self):
